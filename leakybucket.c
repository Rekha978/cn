...
void main()
{
        ...
        for(int i=1; i<=n; i++)
        {
                ...
                if(incoming + stored > bucketSize)
                {
                        ...
                        stored = bucketSize;
                        printf("Bucket overflow! Dropped packet: %d\n", dropped);
                }
                else
                {
                ...
                if(stored > outputRate)
                {
                        ...
                        printf("Transmitted: %d  |  Packet left in bucket: %d\n", outputRate, stored);
                }
                else
                {
                        printf("Transmitted: %d  |  Packet left in bucket: 0\n", stored);
                        stored = 0;
                }
        }
        while(stored > 0)
        {
                ...
                {
                        stored = stored - outputRate;
                        printf("\nTransmitted: %d  |  Packet left in bucket: %d\n", outputRate, stored);
                }
                else
                {
